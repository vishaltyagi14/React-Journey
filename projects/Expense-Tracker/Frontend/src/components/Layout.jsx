...
const Layout = ({ onLogout, user }) => {
  ...
  const fetchTransactions = async () => {
    try {
      ...
      const [incomeRes, expenseRes] = await Promise.all([
        axios.get(`${API_BASE}/api/income/get`, { headers }),
        axios.get(`${API_BASE}/api/expense/get`, { headers }),
      ]);
  ...
  const addTransaction = async (transaction) => {
    try {
      ...
      const headers = token ? { Authorization: `Bearer ${token}` } : {};
      const endpoint =
        transaction.type === "income" ? "api/income/add" : "api/expense/add";
  ...
  const editTransaction = async (id, transaction) => {
    try {
      ...
      const headers = token ? { Authorization: `Bearer ${token}` } : {};
      const endpoint =
        transaction.type === "income" ? "api/income/update" : "api/expense/update";
      ...
      <div className={styles.layout.root}>
        ...
        <div className={styles.layout.mainContainer(sidebarCollapsed)}>
          <div className={styles.header.container}>
            <div>
              <h1 className={styles.header.title}>Financial Overview</h1>
          ...
          <div className={styles.statCards.grid}>
            ...
            <div className={styles.statCards.card}>
              ...
              <p className={styles.statCards.cardFooter}>
                <span className="text-green-600 font-medium">+12.5%</span> Last
                Month
              </p>
            </div>
